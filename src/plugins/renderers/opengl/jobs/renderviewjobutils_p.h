...
#include <opengl_renderer_global_p.h>
...
OPENGL_RENDERER_EXPORT Technique *findTechniqueForEffect(NodeManagers *manager,
                                                    const TechniqueFilter *techniqueFilter,
                                                    Effect *effect);

typedef QVarLengthArray<RenderPass*, 4> RenderPassList;
OPENGL_RENDERER_EXPORT RenderPassList findRenderPassesForTechnique(NodeManagers *manager,
...
template<typename T>
OPENGL_RENDERER_EXPORT inline T variant_value(const QVariant &v)
{
...
OPENGL_RENDERER_EXPORT void parametersFromMaterialEffectTechnique(ParameterInfoList *infoList,
                                                             ParameterManager *manager,
                                                             Material *material,
                                                             Effect *effect,
                                                             Technique *technique);

OPENGL_RENDERER_EXPORT void addParametersForIds(ParameterInfoList *params, ParameterManager *manager,
...
OPENGL_RENDERER_EXPORT ParameterInfoList::const_iterator findParamInfo(ParameterInfoList *infoList,
                                                                  const int nameId);

OPENGL_RENDERER_EXPORT void addToRenderStateSet(RenderStateSet *stateSet,
                                           const QVector<Qt3DCore::QNodeId> stateIds,
                                           RenderStateManager *manager);

typedef QHash<int, QVariant> UniformBlockValueBuilderHash;

struct OPENGL_RENDERER_EXPORT UniformBlockValueBuilder
{
